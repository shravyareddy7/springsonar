...
class MovieServiceImplTest {
    ...
    void testGetMoviesByTheatreId() {
        ...
        assertEquals(2, actualMovies.size()); // Check the size
    ...
    void testSaveMultipleMovies() {
        ...
        assertEquals(1, movies.size()); // Check size after first save

        movieService.saveMovie(movie2);
        movies.add(movie2);

        assertEquals(2, movies.size()); // Check size after second save
    ...
    void testUpdateMovie() {
        ...
        assertNotNull(movieService.getMovieById(movieId)); // Verify that movie exists after update
    }
    ...
    void testDeleteMovie() {
        ...
        // Update the mocked return for movies after deletion
        movies.remove(movie1);

        assertEquals(0, movies.size()); // Check size after deletion
    }
    ...
    void testDeleteMovieById() {
        ...
        // Simulate retrieval of movies after deletion
        Set<Movie> movies = new HashSet<>();
        when(movieDAO.getMoviesByTheatreId(anyInt())).thenReturn(movies);
